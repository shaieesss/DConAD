...
from model.DConvAD import DConvAD
...
class Solver(object):
    ...
    def build_model(self):
        self.model = DConvAD(win_size=self.win_size, enc_in=self.input_c, d_model=self.d_model, e_layers=self.e_layers)
    ...
    def vali(self, vali_loader):
        ...
        loss_1 = []
        loss_2 = []
        for i, (input_x, labels, d_input_x) in enumerate(vali_loader):
            input = input_x.float().to(self.device)
            d_input = d_input_x.float().to(self.device)
            series, prior = self.model(input, d_input)
            series_loss = 0.0
            prior_loss = 0.0
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            self.model.train()
            for i, (input_x, labels, d_input_x) in enumerate(self.train_loader):
    ...
    def test(self):
            ...
            for u in range(len(prior)):
                if u == 0:
                    series_loss = my_kl_loss(series[u], (
                            prior[u] / torch.unsqueeze(torch.sum(prior[u], dim=-1), dim=-1).repeat(1, 1, 1,
                                                                                                   self.d_model)).detach()) * temperature
        ...
        attens_energy = []
        for i, (input_x, labels, d_input_x) in enumerate(self.thre_loader):
            input = input_x.float().to(self.device)
            d_input = d_input_x.float().to(self.device)
            series, prior = self.model(input, d_input)
            series_loss = 0.0
